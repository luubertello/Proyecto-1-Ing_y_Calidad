...
export default function ConsultarImportaciones() {
  ...
  const { setFiltrosNecesarios, valoresFiltros, setValoresFiltros, limpiarFiltros, buscar, setBuscar } =
    useFiltrosContext();
  ...
  useEffect(() => {
    limpiarFiltros();
    setBuscar({ cont: 0, componente: "consultar-importaciones" });
  ...
  useEffect(() => {
    if (buscar.cont > 0 && buscar.componente === "consultar-importaciones") {
      handleBuscarImportaciones(true);
    }
  }, [buscar]);
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      console.error("Error al obtener las importaciones:", err);
      setError("No se pudieron cargar las importacionessss.");
    }
  };
  useEffect(() => {
    fetchData();
  ...
  const handleBuscarImportaciones = async (botonBuscar?: boolean) => {
    if (botonBuscar) {
      resetearPaginacion();
    }
    setLoading(true);
    ...
    const importaciones = await ImportacionService.obtener(filtrosConPaginacion);

    setImportaciones(importaciones.data);
    setEntidadesTotales(importaciones.total);
    setLoading(false);
  };
      ...
      <div className="p-6">
        {loading ? (
          <div className="flex flex-col items-center justify-center py-12">
            ...
            <p className="text-gray-600 dark:text-gray-400 text-lg">Cargando importaciones...</p>
          </div>
        ) : error ? (
          <div className="flex flex-col items-center justify-center py-12">
              ...
              <CardHeader className="flex flex-row items-center justify-between p-4 gap-4">
                ...
                <div className="w-full md:w-auto">
                  <Button
                    ...
                    onClick={() => handleBuscarImportaciones(true)}
                  >
